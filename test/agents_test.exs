defmodule Autopoet.AgentsTest do
  use ExUnit.Case

  test "the research agent is seeded into the body and registered with a narrowed grant" do
    names = Autopoet.Agents.register_from_body() |> Enum.map(&to_string/1)
    assert "researcher" in names

    node = Nexus.Agent.get(:researcher) || Nexus.Agent.get("researcher")
    assert node != nil

    d = Nexus.Agent.def_from_unit(node)
    # grant is net ONLY — the agent browses; it holds nothing else
    assert Enum.map(List.wrap(d[:grant]), &to_string/1) == ["net"]
    # frozen: the autopoet can never autonomously edit its own agent's structure
    assert to_string(Nexus.Agent.management(node)) == "frozen"
    # non-negotiable #3: limits do not exist — agents declare none (fail-mode ceiling only)
