...
function showPower() {
  ...
  s.innerHTML = `
    ...
    <div id="pc-existing"></div>
    <div class="powergrid">
      <div class="powercard reco">
        <div class="pc-badge">recommended</div>
        <div class="pc-name">Workbooks Cloud</div>
        <div class="pc-price">from <b>$20</b><span>/mo</span></div>
        <div class="pc-line">Your own machine — always on. AI, hundreds of tools, a phone number and
          inbox, all included. One flat price; scale up by size.</div>
        <button class="obstepgo" id="pc-cloud">Set up Workbooks Cloud →</button>
      </div>
      <div class="powercard">
        <div class="pc-name">Bring your own AI</div>
        <div class="pc-price"><b>local</b><span> · free</span></div>
        <div class="pc-line">Run on your own OpenRouter key — any model, on your credits. No machine;
          the desktop does the work.</div>
        <input id="pc-orkey" class="obinput" placeholder="sk-or-…  (OpenRouter key)" spellcheck="false" style="margin-top:auto">
        <button class="obstepgo alt" id="pc-local">Use my key →</button>
      </div>
    </div>
    <div class="pc-foot"><span id="pc-status" class="obtext" style="margin:0">Choose one to continue.</span>
      <button class="obstepgo" id="pc-continue" disabled style="opacity:.4">continue →</button></div>`;
  refreshIcons();
  const cont = document.getElementById("pc-continue");
  const setPowered = (ok, msg) => {
    document.getElementById("pc-status").textContent = msg;
    cont.disabled = !ok; cont.style.opacity = ok ? "1" : ".4";
  };
  // CLOUD: expand the INLINE billing panel (machine + tokens + auto-top-up) — one
  // card, right here, no bounce to the dashboard.
  document.getElementById("pc-cloud").onclick = () => showCloudBilling();
  // ALREADY HAVE A NEXUS? list it — connecting it is the zero-cost path
  fetch("/power/cloud/nexuses").then(r => r.json()).then(d => {
    const list = (d && (d.nexuses || d.items)) || (Array.isArray(d) ? d : []);
    if (!list.length) return;
    const box = document.getElementById("pc-existing");
    box.innerHTML = `<div class="powercard reco" style="margin-bottom:14px">
      <div class="pc-badge">your nexus</div>
      <div class="pc-name">Use the nexus you already have</div>
      <div class="pc-line">${list.map(n => `<button class="obstepgo alt pc-nx" data-id="${n.id}">${(n.name || n.id)}${n.plan ? " · " + n.plan : ""} →</button>`).join(" ")}</div>
    </div>`;
    box.querySelectorAll(".pc-nx").forEach(b => b.onclick = () =>
      setPowered(true, `Connected to ${b.textContent.replace(" →", "")} ✓`));
    refreshIcons();
  }).catch(() => {});
  // LOCAL: save the OpenRouter key
  document.getElementById("pc-local").onclick = async () => {
    const inp = document.getElementById("pc-orkey"), k = inp.value.trim();
    if (!k) { inp.focus(); return; }
    const r = await authedPost("/power/openrouter", k);
    setPowered(r.ok, r.ok ? "Local AI (OpenRouter) connected ✓" : "Couldn't save that key — try again.");
  };
  // powered ⇒ an AI is connected ⇒ interactive plan mode (classic quiz is the
  // in-mode fallback link + the unpowered path)
  cont.onclick = () => { if (!cont.disabled) showPlanMode(); };
  // already powered (returning / cloud has a machine)? enable straight away
  fetch("/power/status").then(r => r.json())
    .then(p => { if (p.powered) setPowered(true, p.openrouter ? "Local AI connected ✓" : "Workbooks Cloud connected ✓"); })
    .catch(() => {});
}

// INTERACTIVE PLAN MODE (docs/interactive-plan-mode.md) — lazy-loads elk (1.5MB)
// + planmode.js only when entered, then hands the canvas the same face + hooks
// the quiz gets. onQuiz = the classic fallback; dev entry via settings.
...
async function showCloudBilling() {
  const s = document.getElementById("obsteps");
  s.innerHTML = `
    <div class="obword" style="font-size:22px">Set up Workbooks Cloud</div>
    <p class="obtext" style="margin:0">Your own machine + AI credits, on one card. Auto-top-up keeps it
      running. You can change any of this later.</p>
    <div class="billpanel">
      <div class="bp-row"><label>Machine</label><select id="bp-tier" class="obinput"></select></div>
      <div class="bp-row"><label>Initial AI credits</label><span class="bp-inline">$<input id="bp-credits" class="obinput bp-num" type="number" value="10" min="0" step="5"></span></div>
      <div class="bp-row"><label class="bp-check"><input type="checkbox" id="bp-ato"> Auto-top-up</label>
        <span class="bp-inline" id="bp-ato-fields" style="visibility:hidden">below $<input id="bp-ato-thresh" class="obinput bp-num" type="number" value="5" min="1"> add $<input id="bp-ato-amt" class="obinput bp-num" type="number" value="20" min="5"></span></div>
    </div>
    <div class="pc-foot">
      <button class="obstepgo alt" id="bp-back">← back</button>
      <span id="bp-status" class="obtext" style="margin:0;flex:1;text-align:center"></span>
      <button class="obstepgo" id="bp-pay">Continue to payment →</button>
    </div>`;
  refreshIcons();
  const status = (m) => { document.getElementById("bp-status").textContent = m || ""; };
  document.getElementById("bp-back").onclick = () => showPower();
  document.getElementById("bp-ato").onchange = (e) =>
    document.getElementById("bp-ato-fields").style.visibility = e.target.checked ? "visible" : "hidden";

  // real machine tiers from the cloud (name + price), smallest first
  const tierSel = document.getElementById("bp-tier");
  try {
    const t = await (await fetch("/power/cloud/tiers")).json();
    (t.tiers || []).forEach(tr => {
      const o = document.createElement("option");
      const ram = tr.ram_mb >= 1024 ? (tr.ram_mb / 1024) + "GB" : tr.ram_mb + "MB";
      o.value = tr.id; o.textContent = `${tr.name} — $${tr.price}/mo · ${ram} RAM · ${tr.storage_gb}GB`;
      tierSel.appendChild(o);
    });
    if (!tierSel.options.length) { const o = document.createElement("option"); o.value = "cloud"; o.textContent = "Workbooks Cloud — from $20/mo"; tierSel.appendChild(o); }
  } catch (_) { const o = document.createElement("option"); o.value = "cloud"; o.textContent = "Workbooks Cloud — from $20/mo"; tierSel.appendChild(o); }

  document.getElementById("bp-pay").onclick = async () => {
    const tier = tierSel.value;
    const credits = parseFloat(document.getElementById("bp-credits").value) || 0;
    const ato = document.getElementById("bp-ato").checked;
    document.getElementById("bp-pay").disabled = true;
    status("Preparing checkout…");

    // 1 · save the auto-top-up preference (no charge)
    if (ato) {
      await authedPost("/power/cloud/autotopup", JSON.stringify({
        enabled: true,
        threshold: parseFloat(document.getElementById("bp-ato-thresh").value) || 5,
        amount: parseFloat(document.getElementById("bp-ato-amt").value) || 20
      })).catch(() => {});
    }
    // 2 · machine subscription checkout (captures the card)
    const r = await authedPost("/power/cloud/checkout", JSON.stringify({ plan: tier, initial_credit: credits }));
    const d = await r.json().catch(() => ({}));
    if (!r.ok || !d.url) {
      status(d.detail || d.error || "Couldn't start checkout — is billing configured?");
      document.getElementById("bp-pay").disabled = false;
      return;
    }
    // 3 · open the Polar checkout inline, poll the subscription until active
    openCheckout(d.url, async () => {
      status("Confirming…");
      const ok = await pollPowered(60);
      if (ok) {
        if (credits > 0) { await authedPost("/power/cloud/credits", JSON.stringify({ amount: credits })).catch(() => {}); }
        // back to the gate, now powered → continue
        showPower();
      } else {
        status("Payment not detected yet — finish in the checkout, then retry.");
        document.getElementById("bp-pay").disabled = false;
      }
    });
  };
...
// in-app checkout modal: iframe the Polar page (open-in-browser fallback if the
// provider blocks framing); resolve when the tab is done or the poll succeeds.
function openCheckout(url, onClose) {
  ...
  const done = () => { m.remove(); onClose && onClose(); };
  document.getElementById("bpm-x").onclick = done;
}
...
// the quiz is RETIRED: plan mode is the one path. Its systems (industry
// corpus, pickers) return as plan-mode QUESTION widgets in the brain loop.
